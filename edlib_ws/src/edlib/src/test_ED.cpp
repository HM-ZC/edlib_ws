...
{
    string filename;
    filename = "/home/ali/test3.jpeg";
    Mat originalImg = imread(filename);
    Mat testImg ;
    cvtColor(originalImg,testImg,cv::COLOR_RGB2GRAY);
    // while(testImg.empty()==false)
    // {
    //     imshow("orignal", originalImg);
    ...
    TickMeter tm;
        ...
        Mat LineImgc = colorLine.getLineImage();    //draws on an empty image
        ...
        ed->detectEllipses(colors);

        vector<mCircle> found_ccircles = colorCircle.getCircles();
        vector<mEllipse> found_cellipses = colorCircle.getEllipses();
        Mat colorImg0 = Mat(LineImgc.rows, LineImgc.cols, CV_8UC3, Scalar::all(0));
        Mat colorImg1 = Mat(LineImgc.rows, LineImgc.cols, CV_8UC3, Scalar::all(0));
        ...
        for (int i = 0; i < found_circles.size(); i++)
        {
            ...
            ellipse(colorImg0, center, axes, angle, 0, 360, color, 1, LINE_AA);
        }
        ...
        imwrite("colorImg0.png", colorImg0);
        imwrite("colorImg1.png", colorImg1);
    return 0;
}
